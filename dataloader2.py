...
class MyHeteroData:
    ...
    def preprocess_df(self):
        ...
        self.unique_user_id = pd.DataFrame(
            data={
        ...
        self.unique_movie_id = pd.DataFrame(
            data={
        ...
        self.ratings = self.ratings.drop(
            columns=["userId", "movieId", "timestamp", "imdbId", "tmdbId"]
        )
    ...
    def create_movie_production_edges(self):
        ...
        def create_edge(col_name, node_name):
            ...
            self.production[col_name] = exploded.groupby(exploded.index)[col_name].apply(list)
            edges = []
            ...
            self.data[node_name, "in", "movie"].edge_index = torch.from_numpy(
                edges_array.T
            ).contiguous()
    ...
    def split_data(self):
        transform = T.RandomLinkSplit(
            ...
            # disjoint_train_ratio=0.5, # ignore this make all train edge in supervision and message passing mutually
        ...
        # utils.check_overlap(self.train_data, self.val_data, self.test_data)
